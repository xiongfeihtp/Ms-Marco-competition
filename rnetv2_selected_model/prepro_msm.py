...
def process_file(filename, data_type, word_counter, char_counter):
    ...
    id_list = raw_data['query_id'].keys()
    # sample preprocess
    for id in tqdm(id_list):
        answer = raw_data['answers'][id]
        if answer == []:
            continue

        ques = raw_data['query'][id].replace(
            "''", '" ').replace("``", '" ')
        ...
        for passage in raw_data['passages'][id]:
            if passage['is_selected']:
                context = passage['passage_text'].replace("''", '" ').replace("``", '" ')
                match_score_dict = {}
                if len(answer) > 1:
                    for i, mini_answer in enumerate(answer):
                        match_score_dict[i] = get_match_score(word_tokenize(mini_answer), word_tokenize(context))
                    sorted_score = sorted(match_score_dict.items(), key=lambda item: item[1], reverse=True)
                    match_answer = answer[sorted_score[0][0]]
                else:
                    match_answer = answer[0]

                passage_tokens = word_tokenize(context)
                passage_chars = [list(token) for token in passage_tokens]
                spans = convert_idx(context, passage_tokens)
                ...
                y1s, y2s = [], []
                # 这里做成列表方便计算评估指标
                answer_texts = [match_answer]
                answer_start, answer_end = lcs(context.lower(), match_answer.lower())
                answer_span = []
                temp_span = []
            ...
            eval_examples[str(total)] = {
                "passage_concat": context, "spans": spans, "answers": answer_texts,
                "uuid": raw_data["query_id"][id], "is_selected": passage['is_selected']}
    fh.close()
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False):
    ...
    for example in tqdm(examples):
        ...
        record = tf.train.Example(features=tf.train.Features(feature={
            ...
            "passage_char_idxs": tf.train.Feature(
                bytes_list=tf.train.BytesList(value=[passage_char_idxs.tostring()])),
            "ques_char_idxs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[ques_char_idxs.tostring()])),
            "y1": tf.train.Feature(bytes_list=tf.train.BytesList(value=[y1.tostring()])),
            "y2": tf.train.Feature(bytes_list=tf.train.BytesList(value=[y2.tostring()])),
            "id": tf.train.Feature(int64_list=tf.train.Int64List(value=[example["id"]])),
            "is_selected": tf.train.Feature(int64_list=tf.train.Int64List(value=[int(example["is_selected"])]))
        }))
